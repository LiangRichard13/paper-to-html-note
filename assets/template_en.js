...
(function(){
  ...
  lb.innerHTML = `
    <button class="lb-close" title="Close" type="button">&times;</button>
    <button class="lb-nav lb-prev" title="Previous" type="button">&lsaquo;</button>
    <button class="lb-nav lb-next" title="Next" type="button">&rsaquo;</button>
    <img src="" alt="">
...
(function initMeta(){
  try{
    ...
    const typeLabels={system:'System',algorithm:'Algorithm',survey:'Survey',empirical:'Empirical',position:'Position'};
    ...
    if(ty){const badge=document.querySelector('.top-badge');if(badge)badge.textContent=typeLabels[ty]||ty}
    const labels=[['paper-authors','📄 Authors',a],['paper-institution','🏫 Institution',inst],['paper-method','🔬 Method',mt],['paper-key-finding','🎯 Key Finding',kf]];
...
function showPopup(x,y,forHighlight){
  ...
  if(forHighlight){
    ...
    if(!pop.querySelector('.hl-swatch.delete')){const ds=document.createElement('span');ds.className='hl-swatch delete';ds.title='Delete highlight';ds.onclick=function(e){e.stopPropagation();deleteAnnotation(activeAnnotationId);hidePopup()};pop.appendChild(ds)}
    if(!existingBtn){const eb=document.createElement('button');eb.className='hl-edit-btn';eb.textContent='✎ Note';eb.onclick=function(e){e.stopPropagation();const mk=document.querySelector('mark[data-annotation-id="'+activeAnnotationId+'"]');if(mk){const r=mk.getBoundingClientRect();openStickyEditor(r.right+10,r.top,activeAnnotationId)}hidePopup()};pop.appendChild(eb)}
  } else {
...
function onSelection(e){
  setTimeout(()=>{
    ...
    if(r==='KATEX'){showPopup(e.clientX||e.changedTouches?.[0]?.clientX||100,e.clientY||e.changedTouches?.[0]?.clientY||100,false);showToast('⚠️ Cannot highlight math formulas');setTimeout(hidePopup,1800);return}
...
function renderNotesList(){
  ...
  if(annotations.length===0){
    list.innerHTML='<div class="note-empty"><div class="note-empty-icon">📝</div><p>Highlight text to annotate</p><p style="font-size:11px;margin-top:4px;opacity:.5">Click highlighted text to add notes</p></div>';
  ...
  list.innerHTML=sorted.map(a=>'<div class="note-item" data-id="'+a.id+'" onclick="scrollToAnnotation(\''+a.id+'\')">'+
    ...
    '<div class="note-item-meta"><span class="note-item-time">'+new Date(a.createdAt).toLocaleString('en',{month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit'})+'</span>'+
    '<div class="note-item-actions" onclick="event.stopPropagation()">'+
    '<button onclick="openStickyEditor(event.clientX,event.clientY,\''+a.id+'\')" title="Edit note">✎</button>'+
    '<button class="del-btn" onclick="deleteAnnotation(\''+a.id+'\')" title="Delete">✕</button></div></div></div>').join('');
}
...
function clearAllAnnotationsPrompt(){
  if(!confirm('Clear all highlights and annotations? This cannot be undone.'))return;
...
(function(){
  ...
  //  and reset to false inside saveAnnotations() on any subsequent edit)
  window.addEventListener('beforeunload',function(e){
    if(annotations.length>0 && !window._annotationsPersisted){
      // Custom English confirm — modern browsers ignore e.returnValue text,
      ...
      const msg='You have '+annotations.length+' unsaved annotation(s) on this page.\nLeaving will discard them.\n\nAre you sure you want to leave?';
