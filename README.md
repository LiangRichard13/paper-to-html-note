...
- 🖍 浏览器内荧光笔批注：选中高亮（6 色）、便签编辑、右侧滑动面板管理、手动保存到 HTML（保存按钮在顶栏；未保存退出页面会弹中文确认）
...
模板分为三部分，CSS 和 JS 从 `template.html` 中独立提取为规范文件：

| 文件 | 作用 | 生成时处理 |
|------|------|-----------|
| `template.html` | HTML 骨架（`<!-- STYLESHEET -->` / `<!-- SCRIPT -->` 占位符） | 被 CSS/JS 内容替换 |
| `template.css` | 所有样式（含批注系统）的**规范来源** | Pipeline A：A4.1 Python 内联 |
| `template.js` | 所有脚本（含批注引擎）的**规范来源** | Pipeline B：B4c 读取后填入占位符 |

**铁则**：Agent 在任何情况下不得手写 CSS 或 JS。所有样式和脚本必须从 `template.css` / `template.js` 读取。完整自包含模板（含内联 CSS/JS）见 `template.html.bak`，可用于独立预览。
...
24 个组件的完整模板与使用指南见 `references/component-catalog.md`。核心组件：
...
详情见 `SKILL.md` 中的 Note Indexing 节。参考文件（策略定义、质量检查、子代理模板等）见 `references/` 目录。
...
```
paper-to-html-note/
  SKILL.md                     # 核心指令流程（拆分后 ~1350 行）
  README.md
  LICENSE
  pyproject.toml
  assets/
    README_en.md
    template.html              # HTML 骨架（含 <!-- STYLESHEET --> / <!-- SCRIPT --> 占位符）
    template.css               # 规范 CSS 来源（所有 agent 从此读，禁止手写）
    template.js                # 规范 JS 来源（含批注引擎）
    template.html.bak          # 完整自包含模板备份（独立预览用）
    template_en.html           # 英文 HTML 骨架
    template_en.css            # 英文 CSS
    template_en.js             # 英文 JS
    template_en.html.bak       # 英文完整备份
    index-template.html        # 笔记索引页模板
    examples/
      screenshot.png
      toward_personalized_llm_powered_agents_reading_notes.html
  references/
    component-catalog.md       # 24 个 HTML 组件完整模板
    content-depth-rules.md     # 深度规则（倒金字塔、洞察类型、双层结构）
    design-system.md           # CSS 变量与设计系统
    organization-strategies.md # 四种组织策略完整定义
    quality-checks.md          # 29 项 A4.8 质量检查清单
    subagent-prompt-reference.md # Pipeline B 子代理提示模板全集
  scripts/
    extract_figures.py
    assemble_figures.py
